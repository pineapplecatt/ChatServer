...
Server::Server() {
    ...
    serverAddr.sin_port = htons(SERVER_PORT);           // 端口号，转为网络号
    serverAddr.sin_addr.s_addr = inet_addr(SERVER_IP);  // IP地址，转为网络字节序
    // listener
    listener = 0;
    // epoll fd
    epfd = 0;
...
int Server::sendBroadcastMessage(int clientfd) {
    ...
    } else {  // send the message broadcast
...
void Server::start() {
    ...
    while (1) {
        ...
        int epoll_events_count = epoll_wait(epfd, events, EPOLL_SIZE, -1);//等待epoll文件描述符上的I/O事件
